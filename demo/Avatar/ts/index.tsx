...
const handleErrors: TOnError = (err) => {
    console.log('err', err);
    if (!Array.isArray(err)) alert((err.message as any)?.responseBody?.message || err.message);
    else {
...
type TGetUploadParamsExt = (
    localFileData: ILocalFileData | ILocalFileData[]
) => ReturnType<TGetUploadParams>;
export const uploadParams: TGetUploadParamsExt = (localFileData) => {
    return {
        URL: `/api/avatar`,
...
type TDeleteFile = (localFileData: IRemoteFileData) => Promise<void>;
const deleteDocument: TDeleteFile = (fileData) =>
...
const overrideFn: (dropZoneClassName: string, avatarColor?: string) => TOverrides = (
    dropZoneClassName,
    avatarColor
) => ({
    Root: {
